...
if not _jwt_secret:
    ...
    logger.warning(
        "No JWT_SECRET_KEY set - using ephemeral key. Tokens will be invalid after restart."
    )
...
def create_refresh_token(user_id, device_info=None):
    ...
    expires_at = (
        datetime.datetime.now(datetime.timezone.utc) + JWT_REFRESH_TOKEN_EXPIRES
    )
...
def check_tier_limit(user, feature: str) -> tuple[bool, dict]:
    ...
    if feature == "bills":
        ...
        if is_saas():
            ...
            used = (
                Bill.query.join(Database)
                .filter(Database.owner_id == user.id, Bill.archived == False)
                .count()
            )
        else:
        ...
        if is_saas():
            ...
            pending_invites = (
                UserInvite.query.filter_by(invited_by_id=user.id)
                .filter(
...
def calculate_next_due_date(
    current_due, frequency, frequency_type="simple", frequency_config=None
):
    if frequency_config is None:
        frequency_config = {}
    if isinstance(current_due, str):
        current_date = datetime.date.fromisoformat(current_due)
    else:
    ...
    if frequency == "weekly":
        ...
        if next_days:
            return current_date + timedelta(days=min(next_days) - current_weekday)
        else:
...
def jwt_logout_all():
    ...
    RefreshToken.query.filter_by(user_id=g.jwt_user_id, revoked=False).update(
        {"revoked": True}
    )
    db.session.commit()
...
def billing_config():
    ...
    return jsonify({"success": True, "publishable_key": STRIPE_PUBLISHABLE_KEY})
...
def create_checkout():
    ...
    if (
        ...
        and not user.subscription.is_trialing
    ):
        return jsonify(
    ...
    if interval not in ("monthly", "annual"):
        return jsonify(
            {"success": False, "error": "Invalid interval. Must be monthly or annual"}
        ), 400
    ...
    return jsonify(
        {
            "success": True,
            "data": {"url": result["url"], "session_id": result["session_id"]},
        }
    )
...
def billing_portal():
    ...
    return jsonify({"success": True, "data": {"url": result["url"]}})
...
def change_plan():
    ...
    if user.subscription.status not in ("active", "past_due"):
        return jsonify(
            {"success": False, "error": "Subscription must be active to change plans"}
        ), 400
    ...
    if not new_price_id:
        return jsonify(
            {"success": False, "error": "Price not configured for selected plan"}
        ), 400
    ...
    result = update_subscription(
        user.subscription.stripe_subscription_id, new_price_id, prorate=is_upgrade
    )
...
def jwt_delete_account():
    ...
    if account_user_ids:
        ...
        TwoFAChallenge.query.filter(
            TwoFAChallenge.user_id.in_(account_user_ids)
...
def jwt_get_bills():
    ...
    if owned_bill_ids:
        share_count_query = (
            db.session.query(BillShare.bill_id, func.count(BillShare.id).label("count"))
            .filter(
                BillShare.bill_id.in_(owned_bill_ids),
                BillShare.status.in_(["pending", "accepted"]),
            )
            .group_by(BillShare.bill_id)
            .all()
        )
    ...
    for bill in shared_bills_data:
        ...
        owner = (
            db.session.get(User, database_owner.owner_id) if database_owner else None
        )
...
def jwt_create_bill():
    ...
    if "database_id" in data:
        ...
        return jsonify(
            {
                "success": False,
                "error": "database_id is required when creating from All Buckets view",
            }
        ), 400
    else:
        target_db = Database.query.filter_by(name=g.jwt_db_name).first()
...
def jwt_update_bill(bill_id):
    ...
    if "varies" in data:
        bill.is_variable = data["varies"]
    if "frequency" in data:
        bill.frequency = data["frequency"]
    if "frequency_type" in data:
        bill.frequency_type = data["frequency_type"]
    if "frequency_config" in data:
        bill.frequency_config = data["frequency_config"]
    if "next_due" in data:
        bill.due_date = data["next_due"]
    if "auto_payment" in data:
        bill.auto_pay = data["auto_payment"]
    if "icon" in data:
        bill.icon = data["icon"]
    if "type" in data:
        bill.type = data["type"]
...
def jwt_get_all_payments():
    ...
    owned_payments = (
        db.session.query(Payment)
        .join(Bill)
        .filter(Bill.database_id.in_(accessible_db_ids))
        .all()
    )
    ...
    shared_payments = (
        db.session.query(Payment)
        .join(BillShare, Payment.share_id == BillShare.id)
        .filter(
...
def jwt_share_bill(bill_id):
    ...
    if existing:
        return jsonify(
            {"success": False, "error": "Bill already shared with this user"}
        ), 400
...
def jwt_get_shared_bills():
    ...
    shares = BillShare.query.filter_by(
        shared_with_user_id=g.jwt_user_id, status="accepted"
    ).all()
...
def jwt_get_stats_by_account():
    ...
    results = (
        db.session.query(Bill.account, func.sum(Payment.amount), Bill.type)
        .join(Payment)
        .filter(
            Bill.database_id.in_(accessible_db_ids),
            Bill.account != None,
        ...
        .group_by(Bill.account, Bill.type)
        .all()
    )
    ...
    for r in results:
        ...
        by_account[account]["total"] = (
            by_account[account]["expenses"] - by_account[account]["deposits"]
        )
    ...
    result = [
        {"account": k, **v}
        for k, v in sorted(
            by_account.items(), key=lambda x: x[1]["expenses"], reverse=True
        )
    ]
...
def jwt_get_stats_yearly():
    ...
    results = (
        db.session.query(
            func.to_char(
                func.to_date(Payment.payment_date, "YYYY-MM-DD"), "YYYY"
            ).label("year"),
            func.sum(Payment.amount),
            Bill.type,
        )
...
def jwt_get_monthly_comparison():
    ...
    results = (
        db.session.query(
            ...
            func.to_char(func.to_date(Payment.payment_date, "YYYY-MM-DD"), "MM").label(
                "month"
            ),
            func.sum(Payment.amount),
            Bill.type,
        ...
        .filter(
            Bill.database_id.in_(accessible_db_ids),
            func.to_char(func.to_date(Payment.payment_date, "YYYY-MM-DD"), "YYYY").in_(
                [str(current_year), str(last_year)]
            ),
        )
        .group_by("year", "month", Bill.type)
        .all()
    )
...
def jwt_process_auto_payments():
    ...
    return jsonify(
        {
            "success": True,
            "data": {"processed_count": len(processed), "bills": processed},
        }
    )
...
def _generate_apple_client_secret():
    ...
    # Normalize env formatting (quoted, escaped, CRLF).
        ...
        if existing_user:
            ...
            if existing_user.email_verified_at:
                ...
                return (
                    None,
                    False,
                    "An account with this email exists but is not verified. Please verify your email first or log in with your password.",
                )
...
def oauth_authorize(provider):
    ...
    if provider not in get_enabled_oauth_providers():
        return jsonify(
            {"success": False, "error": f'Provider "{provider}" is not enabled'}
        ), 400
    ...
    if not auth_endpoint:
        return jsonify(
            {"success": False, "error": "Provider missing authorization endpoint"}
        ), 502
...
def oauth_callback(provider):
    ...
    if not token_endpoint:
        return jsonify(
            {"success": False, "error": "Provider missing token endpoint"}
        ), 502
...
def oauth_unlink_provider(provider):
    ...
    account = OAuthAccount.query.filter_by(
        user_id=g.jwt_user_id, provider=provider
    ).first()

    if not account:
        return jsonify(
            {"success": False, "error": f"No {provider} account linked"}
        ), 404

    # Prevent unlinking if user has no password (would lock them out)
    if not user.password_hash:
        other_accounts = OAuthAccount.query.filter(
            OAuthAccount.user_id == g.jwt_user_id, OAuthAccount.provider != provider
        ).count()
        if other_accounts == 0:
            return jsonify(
                {
    ...
    return jsonify(
        {"success": True, "data": {"message": f"{provider} account unlinked"}}
    )
...
def _verify_2fa_session(token):
    ...
    if not token:
        return None, (
            jsonify({"success": False, "error": "Missing 2FA session token"}),
            400,
        )
    ...
    if not challenge:
        return None, (
            jsonify({"success": False, "error": "Invalid or expired 2FA session"}),
            401,
        )
    ...
    if challenge.used:
        return None, (
            jsonify({"success": False, "error": "2FA session already used"}),
            401,
        )
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                "passkeys": passkeys,
                "has_recovery_codes": bool(config.recovery_codes_hash)
                if config
                else False,
            },
        }
...
def twofa_setup_email():
    ...
    if not user.email:
        return jsonify(
            {
                "success": False,
                "error": "You must have an email address to enable email OTP",
            }
        ), 400
...
def twofa_get_recovery_codes():
    ...
    return jsonify({"success": True, "data": {"recovery_codes": codes}})
...
def twofa_passkey_register():
    ...
    if not registration_token or not credential:
        return jsonify(
            {"success": False, "error": "Missing registration_token or credential"}
        ), 400
    ...
    try:
        ...
        credential_payload = {
            ...
            "response": {
                ...
                "attestationObject": credential.get("response", {}).get(
                    "attestationObject"
                )
                if isinstance(credential.get("response"), dict)
                else None,
...
def twofa_delete_passkey(passkey_id):
    ...
    if user.password_hash:
        if not password:
            return jsonify(
                {"success": False, "error": "Password required to delete passkey"}
            ), 400
        ...
        confirm_challenge = (
            ...
            .order_by(TwoFAChallenge.created_at.desc())
            .first()
        )
    ...
    cred = WebAuthnCredential.query.filter_by(
        id=passkey_id, user_id=g.jwt_user_id
    ).first()
    if not cred:
...
def twofa_disable():
    ...
    if user.password_hash:
        if not password:
            return jsonify(
                {"success": False, "error": "Password required to disable 2FA"}
            ), 400
        if not user.check_password(password):
            return jsonify({"success": False, "error": "Invalid password"}), 401
    else:
        ...
        if not disable_challenge or not disable_challenge.is_valid:
            return jsonify(
                {
                    "success": False,
                    "error": "Invalid or expired confirmation code. Request a new one.",
                }
            ), 401
...
def twofa_disable_send_code():
    ...
    if user.password_hash:
        return jsonify(
            {"success": False, "error": "Use password confirmation instead"}
        ), 400
    ...
    if not sent:
        return jsonify(
            {"success": False, "error": "Failed to send confirmation code"}
        ), 502

    return jsonify(
        {"success": True, "data": {"message": "Confirmation code sent to your email"}}
    )
...
def twofa_challenge():
    ...
    if method == "email_otp":
        if not user.email:
            return jsonify(
                {"success": False, "error": "No email address configured"}
            ), 400
...
def twofa_passkey_auth_options():
    ...
    return jsonify({"success": True, "data": {"options": options_json}})
...
def twofa_verify():
    ...
    if method == "email_otp":
        ...
        stored_cred = WebAuthnCredential.query.filter_by(
            user_id=challenge.user_id, credential_id=cred_id_from_response
        ).first()
        ...
        if not config or not config.recovery_codes_hash:
            return jsonify(
                {"success": False, "error": "No recovery codes available"}
            ), 400
        ...
        return jsonify(
            {"success": False, "error": f"Unknown 2FA method: {method}"}
        ), 400
...
def jwt_create_user():
    ...
    # Create the new user
    new_user = User(
        username=username, role=role, email=email, password_change_required=True
    )
    if is_saas():
        new_user.created_by_id = current_user_id
    new_user.set_password(password)
            ...
            "data": [
                {
                    ...
                    "email": inv.email,
                    "role": inv.role,
                    "database_ids": [int(x) for x in inv.database_ids.split(",") if x]
                    if hasattr(inv, "database_ids") and inv.database_ids
                    else [],
                    "created_at": inv.created_at.isoformat(),
                    "expires_at": inv.expires_at.isoformat(),
                }
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                if email_sent
                else "Failed to resend invitation"
            },
        }
    )
...
def jwt_create_database():
    ...
    if Database.query.filter_by(name=name).first():
        return jsonify(
            {"success": False, "error": "A database with this name already exists"}
        ), 400
...
@api_v2_bp.route(
    "/databases/<int:database_id>/access/<int:target_user_id>", methods=["DELETE"]
)
@jwt_admin_required
...
def send_test_notification():
    ...
    if not is_push_enabled():
        return jsonify(
            {
                "success": False,
                "error": "Push notifications not configured (FCM_SERVER_KEY missing)",
            }
        ), 503
    ...
    sent_count = send_push_to_user(
        g.jwt_user_id, title, body, {"action": "test"}, notification_type="test"
    )
...
def cleanup_expired_shares():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Failed to cleanup expired shares"}
        ), 500
...
def notify_payment_recorded(
    user_id: int, bill_id: int, bill_name: str, amount: float, payment_date: str
):
...
def jwt_get_devices():
    ...
    devices = (
        UserDevice.query.filter_by(user_id=g.jwt_user_id)
        .order_by(desc(UserDevice.last_active_at))
        .all()
    )
...
def jwt_register_device():
    ...
    if not device_id or not platform:
        return jsonify(
            {"success": False, "error": "device_id and platform are required"}
        ), 400
    ...
    if platform not in ("ios", "android", "web", "desktop"):
        return jsonify(
            {
                "success": False,
                "error": "Invalid platform. Must be ios, android, web, or desktop",
            }
        ), 400
...
def jwt_sync_push():
    ...
    for bill_data in data.get("bills", []):
        ...
        if bill_id:
            ...
            if "varies" in bill_data:
                bill.is_variable = bill_data["varies"]
            if "frequency" in bill_data:
                bill.frequency = bill_data["frequency"]
            if "frequency_type" in bill_data:
                bill.frequency_type = bill_data["frequency_type"]
            if "frequency_config" in bill_data:
                bill.frequency_config = bill_data["frequency_config"]
            if "next_due" in bill_data:
                bill.due_date = bill_data["next_due"]
            if "auto_payment" in bill_data:
                bill.auto_pay = bill_data["auto_payment"]
            if "icon" in bill_data:
                bill.icon = bill_data["icon"]
            if "type" in bill_data:
            ...
            accepted_bills.append(
                {
                    "id": new_bill.id,
                    "action": "created",
                    "client_ref": bill_data.get(
                        "client_ref"
                    ),  # For client to map temp ID
                }
            )
...
def jwt_sync():
    ...
    if not since_str:
        return jsonify(
            {"success": False, "error": "since parameter is required (ISO timestamp)"}
        ), 400
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Invalid since timestamp format. Use ISO 8601."}
        ), 400
    ...
    bill_query = Bill.query.filter(
        Bill.database_id == target_db.id, Bill.last_updated > since
    )
    if not include_archived:
    ...
    payments = (
        Payment.query.filter(
            Payment.bill_id.in_(all_bill_ids), Payment.updated_at > since
        ).all()
        if all_bill_ids
        else []
    )
...
def jwt_sync_full():
    ...
    payments = (
        Payment.query.filter(Payment.bill_id.in_(bill_ids)).all() if bill_ids else []
    )
...
def create_app():
    ...
    enable_telemetry_receiver = (
        os.environ.get("ENABLE_TELEMETRY_RECEIVER", "false").lower() == "true"
    )
    if enable_telemetry_receiver:
        ...
        logger.info(
            "📊 Telemetry receiver enabled - this server will collect installation statistics"
        )
    ...
    with app.app_context():
        try:
            ...
            is_fresh_install = (
                "schema_migrations" not in inspector.get_table_names()
            ...
            if is_fresh_install:
                ...
                logger.info(
                    f"✨ Fresh install - marked {len(MIGRATIONS)} migrations as applied (schema already current)"
                )
            else:
            ...
            if user_count == 0:
                logger.info(
                    "🚀 First run detected - creating default admin and database"
                )
                # Generate secure random password for first admin
                initial_password = secrets.token_urlsafe(12)
                admin = User(
                    username="admin", role="admin", password_change_required=True
                )
                ...
                p_db = Database(
                    name="personal",
                    display_name="Personal Finances",
                    description="Personal bills and expenses",
                )
                db.session.add(p_db)
                ...
                logger.info(
                    f"📦 Existing installation detected ({user_count} users) - skipping default creation"
                )
        except Exception as e:
